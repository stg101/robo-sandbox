...
GLFWwindow *GuiMgr::g_mainWindow = nullptr;
int32 GuiMgr::s_testSelection = 0;
Settings GuiMgr::s_settings;
Test *GuiMgr::s_test = nullptr;
bool GuiMgr::s_rightMouseDown = false;
b2Vec2 GuiMgr::s_clickPointWS = b2Vec2_zero;
...
void GuiMgr::process(const float dt)
{
    ...
    if (g_debugDraw.m_showUI)
    {
        sprintf(buffer, "%.1f ms", 1000.0 * dt);
...
void GuiMgr::glfwErrorCallback(int error, const char *description)
{
    fprintf(stderr, "GLFW error occured. Code: %d. Description: %s\n", error, description);
}

void GuiMgr::CreateUI(GLFWwindow *window, const char *glslVersion)
{
    IMGUI_CHECKVERSION();
    ImGui::CreateContext();
...
void GuiMgr::ResizeWindowCallback(GLFWwindow *, int width, int height)
{
...
void GuiMgr::KeyCallback(GLFWwindow *window, int key, int scancode, int action, int mods)
{
...
void GuiMgr::CharCallback(GLFWwindow *window, unsigned int c)
{
    ImGui_ImplGlfw_CharCallback(window, c);
}

void GuiMgr::MouseButtonCallback(GLFWwindow *window, int32 button, int32 action, int32 mods)
{
...
void GuiMgr::MouseMotionCallback(GLFWwindow *, double xd, double yd)
{
    b2Vec2 ps((float)xd, (float)yd);
...
void GuiMgr::ScrollCallback(GLFWwindow *window, double dx, double dy)
{
...
void GuiMgr::SortTests()
{
    std::sort(g_testEntries, g_testEntries + g_testCount, CompareTests);
}

inline bool GuiMgr::CompareTests(const TestEntry &a, const TestEntry &b)
{
